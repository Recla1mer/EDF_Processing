...
PICKLE_DIRECTORY_NAME = "PICKLE_DIRECTORY_NAME"
FIGURE_DIRECTORY_PATH = "Figures/"
# if not os.path.isdir(FIGURE_DIRECTORY_PATH):
#     os.mkdir(FIGURE_DIRECTORY_PATH)

example_x = [1, 2, 3, 4, 5]
example_y = [1, 4, 9, 16, 25]
example_y_2 = [1, 8, 27, 64, 125]
example_z = [1, 2, 4, 8, 10]
min_z = 1
max_z = 10
...
def simple_plot(pickle_name = "path.pkl", **kwargs):
    """
    """
    ...
    kwargs.setdefault("legend", ["y_1", "y_2"])
    ...
    ax.plot(example_x, example_y, label=kwargs["legend"][0])
    ax.plot(example_x, example_y_2, label=kwargs["legend"][1])
    ax.legend(loc="best")
...
def colorbar_scatter_plot(pickle_name="path.pkl", **kwargs):
    """
    ...
    im = ax.scatter(
        example_x, example_y, c=example_z, cmap=color, vmin=min_z, vmax=max_z, marker=kwargs["marker"], s=kwargs["s"]
    )
